...
# Taken from: https://stackoverflow.com/questions/25200220/generate-a-random-derangement-of-a-list
...
def main_train():
    ...
    # Other
    parser.add_argument('--positive_weighting_factor', type=float, default=0.5, help='the value of the factor that weights positive and negative sampling')
    ...
    if args.src_vocabulary is not None:
        ...
        src_dictionary = data_contrastive.Dictionary(src_words)
    if args.trg_vocabulary is not None:
        ...
        trg_dictionary = data_contrastive.Dictionary(trg_words)
    if args.src_embeddings is not None:
        f = open(args.src_embeddings, encoding=args.encoding, errors='surrogateescape')
        src_embeddings, src_dictionary = data_contrastive.read_embeddings(f, args.cutoff, src_words)
    ...
    if args.trg_embeddings is not None:
        ...
        trg_embeddings, trg_dictionary = data_contrastive.read_embeddings(trg_file, args.cutoff, trg_words)
    ...
    if args.learn_encoder_embeddings:
        src_encoder_embeddings = device(data_contrastive.random_embeddings(src_dictionary.size(), embedding_size))
        trg_encoder_embeddings = device(data_contrastive.random_embeddings(trg_dictionary.size(), embedding_size))
    ...
    if args.fixed_decoder_embeddings:
        ...
        src_decoder_embeddings = device(data_contrastive.random_embeddings(src_dictionary.size(), embedding_size))
        trg_decoder_embeddings = device(data_contrastive.random_embeddings(trg_dictionary.size(), embedding_size))
    ...
    # Build translators
    src2src_translator = ContrastiveTranslator(encoder_embeddings=src_encoder_embeddings,
    ...
    src2trg_translator = ContrastiveTranslator(encoder_embeddings=src_encoder_embeddings,
    ...
    trg2trg_translator = ContrastiveTranslator(encoder_embeddings=trg_encoder_embeddings,
                                    decoder_embeddings=trg_decoder_embeddings, generator=trg_generator,
                                    src_dictionary=trg_dictionary, trg_dictionary=trg_dictionary, encoder=encoder,
                                    decoder=trg_decoder, denoising=not args.disable_denoising, device=device)
    trg2src_translator = ContrastiveTranslator(encoder_embeddings=trg_encoder_embeddings,
                                    decoder_embeddings=src_decoder_embeddings, generator=src_generator,
    ...
    if args.src is not None:
        ...
        corpus = data_contrastive.CorpusReader(f, max_sentence_length=args.max_sentence_length, cache_size=args.cache)
        ...
        if not args.disable_backtranslation:
            trgback2src_trainer = ContrastiveTrainer(translator=trg2src_translator, optimizers=trg2src_optimizers,
                                          corpus=data_contrastive.BacktranslatorCorpusReader(corpus=corpus, translator=src2trg_translator), batch_size=args.batch, lambda_factor=args.positive_weighting_factor)
    ...
    if args.trg is not None:
        ...
        if not args.disable_backtranslation:
            srcback2trg_trainer = ContrastiveTrainer(translator=src2trg_translator, optimizers=src2trg_optimizers,
                                          corpus=data_contrastive.BacktranslatorCorpusReader(corpus=corpus, translator=trg2src_translator), batch_size=args.batch, lambda_factor=args.positive_weighting_factor)
    ...
    if args.src2trg is not None:
        ...
        corpus = data_contrastive.CorpusReader(f1, f2, max_sentence_length=args.max_sentence_length, cache_size=args.cache if args.cache_parallel is None else args.cache_parallel)
...
class Trainer:
    ...
    def step(self):
        ...
        self.src_word_count += sum([len(data_contrastive.tokenize(sentence)) + 1 for sentence in src])  # TODO Depends on special symbols EOS/SOS
        self.trg_word_count += sum([len(data_contrastive.tokenize(sentence)) + 1 for sentence in trg])  # TODO Depends on special symbols EOS/SOS
        self.io_time += time.time() - t

        # Compute loss
        t = time.time()
        loss, _ = self.translator.score(src, trg, train=True)
...
class ContrastiveTrainer:
    def __init__(self, corpus, optimizers, translator, batch_size=50, lambda_factor=0.5):
        self.corpus = corpus
        self.translator = translator
        self.optimizers = optimizers
        self.batch_size = batch_size
    ...
    def step(self):
        ...
        t = time.time()
        src, trg = self.corpus.next_batch(self.batch_size)
        self.src_word_count += sum(
            [len(data_contrastive.tokenize(sentence)) + 1 for sentence in src])  # TODO Depends on special symbols EOS/SOS
        self.trg_word_count += sum(
            [len(data_contrastive.tokenize(sentence)) + 1 for sentence in trg])  # TODO Depends on special symbols EOS/SOS
        self.io_time += time.time() - t
        ...
        positive_loss = self.contrastive_criterion(flattened_hidden, flattened_corpus_hidden, torch.ones(flattened_hidden.shape[0]))
        ...
        negative_loss = self.contrastive_criterion(flattened_hidden, flattened_hidden[negative_indices], -1 * torch.ones(flattened_hidden.shape[0]))
        negative_loss += self.contrastive_criterion(flattened_corpus_hidden, flattened_corpus_hidden[negative_indices], -1 * torch.ones(flattened_corpus_hidden.shape[0]))
...
class Validator:
    def __init__(self, translator, source, reference, batch_size=50, beam_size=0):
        ...
        self.reference_word_count = sum([len(data_contrastive.tokenize(sentence)) + 1 for sentence in self.reference])  # TODO Depends on special symbols EOS/SOS
        self.batch_size = batch_size
        self.beam_size = beam_size

        # Sorting
        lengths = [len(data_contrastive.tokenize(sentence)) for sentence in self.source]
