...
from undreamt import data_contrastive
...
class EmbeddingGenerator(nn.Module):
    def __init__(self, hidden_size, embedding_size):
        ...
        self.special_out = nn.Linear(embedding_size, data_contrastive.SPECIAL_SYMBOLS, bias=False)
...
class WrappedEmbeddingGenerator(nn.Module):
    ...
    def output_classes(self):
        return self.embeddings.weight.data.size()[0] + data_contrastive.SPECIAL_SYMBOLS - 1


class LinearGenerator(nn.Module):
    def __init__(self, hidden_size, vocabulary_size, bias=True):
        super(LinearGenerator, self).__init__()
        self.out = nn.Linear(hidden_size, data_contrastive.SPECIAL_SYMBOLS + vocabulary_size, bias=bias)
